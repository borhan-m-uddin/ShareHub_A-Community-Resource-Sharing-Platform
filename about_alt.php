...
if (db_connected()) {
    try {
        if ($st = $conn->prepare("SELECT item_id, title, posting_date FROM items WHERE availability_status='available' ORDER BY posting_date DESC LIMIT 5")) {
            ...
            while ($r = $res->fetch_assoc()) $latest_items[] = $r; $st->close();
        }
        if ($st = $conn->prepare("SELECT service_id, title, posting_date FROM services WHERE availability='available' ORDER BY posting_date DESC LIMIT 5")) {
            $st->execute(); $res = $st->get_result();
            while ($r = $res->fetch_assoc()) $latest_services[] = $r; $st->close();
        }

        $items_count=0; $services_count=0; $users_count=0; $requests_today=0; $requests_week=0; $fulfilled_rate=null;

        if ($st = $conn->prepare("SELECT COUNT(*) c FROM items WHERE availability_status='available'")) { $st->execute(); $r=$st->get_result()->fetch_assoc(); $items_count=(int)($r['c']??0); $st->close(); }
        if ($st = $conn->prepare("SELECT COUNT(*) c FROM services WHERE availability='available'")) { $st->execute(); $r=$st->get_result()->fetch_assoc(); $services_count=(int)($r['c']??0); $st->close(); }
        if ($st = $conn->prepare("SELECT COUNT(*) c FROM users WHERE status=1")) { $st->execute(); $r=$st->get_result()->fetch_assoc(); $users_count=(int)($r['c']??0); $st->close(); }

        if ($st = $conn->prepare("SELECT COUNT(*) c FROM requests WHERE DATE(created_at)=CURDATE()")) { if($st->execute()){ $r=$st->get_result()->fetch_assoc(); $requests_today=(int)($r['c']??0);} $st->close(); }
        if ($st = $conn->prepare("SELECT COUNT(*) c FROM requests WHERE YEARWEEK(created_at,1)=YEARWEEK(CURDATE(),1)")) { if($st->execute()){ $r=$st->get_result()->fetch_assoc(); $requests_week=(int)($r['c']??0);} $st->close(); }
        if ($st = $conn->prepare("SELECT SUM(CASE WHEN status='completed' THEN 1 ELSE 0 END) done, COUNT(*) total FROM requests")) {
            if($st->execute()){ $r=$st->get_result()->fetch_assoc(); $done=(int)($r['done']??0); $total=(int)($r['total']??0); $fulfilled_rate = $total>0 ? round(($done/$total)*100) : null; }
            $st->close();
        }

        if ($st = $conn->prepare("SELECT COUNT(*) c FROM items WHERE availability_status='available' AND posting_date >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)")) { if($st->execute()){ $r=$st->get_result()->fetch_assoc(); $items_new_week=(int)($r['c']??0);} $st->close(); }
        if ($st = $conn->prepare("SELECT COUNT(*) c FROM services WHERE availability='available' AND posting_date >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)")) { if($st->execute()){ $r=$st->get_result()->fetch_assoc(); $services_new_week=(int)($r['c']??0);} $st->close(); }
...
<head>
    ...
    <style>
        /* Hero: gradient mesh */
        .alt-wrapper { max-width: 1100px; margin: 20px auto; padding: 0 22px; }
        .alt-hero { position:relative; overflow:hidden; border-radius:16px; padding:56px 22px; box-shadow:0 14px 48px rgba(5,41,45,.16); }
        .alt-hero::before{
        ...
        .alt-hero .inner{ position:relative; z-index:1; display:grid; grid-template-columns:1fr; gap:22px; max-width:960px; }
        .alt-hero h1{ font-size:2.6rem; margin:0 0 12px; letter-spacing:.2px; }
        .alt-hero p.lead{ font-size:1.08rem; opacity:.95; margin:0 0 18px; }
        .alt-hero .ctas a{ margin-right:10px; }

        /* Compact stat widgets (replacing old cluster) */
        .mini-widgets{ display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:12px; margin-top:12px; }
        .mini{ background:var(--card); border:1px solid var(--border); border-radius:14px; box-shadow:var(--shadow); padding:14px; position:relative; overflow:hidden; }
        .mini::before{ content:""; position:absolute; inset:0; background:linear-gradient(90deg, rgba(35,211,179,.12), transparent 60%); pointer-events:none; }
        .mini h5{ margin:0 0 6px; font-size:1rem; }
        .mini p{ margin:0; color:var(--muted); font-weight:600; }
        @media (max-width: 900px){ .mini-widgets{ grid-template-columns:1fr; } }
        ...
        .tile{ background:var(--card); border:1px solid var(--border); border-radius:14px; padding:18px; box-shadow:var(--shadow); transition:transform .18s ease, box-shadow .22s ease, border-color .22s ease; position:relative; overflow:hidden; }
        .tile:hover{ transform:translateY(-4px); box-shadow:0 16px 32px rgba(5,41,45,.12); border-color:rgba(37,99,235,.18); }
        .tile::before{ content:""; position:absolute; inset:0; background:radial-gradient(400px 140px at -10% -10%, rgba(35,211,179,.15), transparent 70%); pointer-events:none; }
        .tile h4{ margin:6px 0 8px; }
        .tile p{ margin:0; color:var(--muted); }
        .tile .icon{ width:40px; height:40px; border-radius:10px; display:inline-grid; place-items:center; background:rgba(35,211,179,.12); color:var(--primary); }
        .tile ul{ margin:8px 0 0; padding-left:18px; color:var(--muted); }
        .tile a.more{ display:inline-block; margin-top:10px; font-weight:700; }

        /* Ticker */
        .ticker{ margin-top:28px; overflow:hidden; border:1px solid var(--border); border-radius:12px; background:var(--card); }
        .ticker ul{ display:flex; gap:24px; list-style:none; padding:10px 16px; margin:0; animation:tickerMove 28s linear infinite; }
        .ticker li{ white-space:nowrap; color:var(--muted); }
        ...
        .cta-band{ margin-top:32px; padding:18px; border-radius:14px; color:#fff; background:linear-gradient(120deg, var(--primary) 0%, var(--laser) 50%, var(--info) 100%); box-shadow:0 12px 32px rgba(5,41,45,.18); display:flex; align-items:center; justify-content:space-between; gap:12px; flex-wrap:wrap; }
        ...
        .step .num{ width:36px; height:36px; border-radius:50%; display:inline-grid; place-items:center; font-weight:900; color:#fff; background:var(--primary); box-shadow:0 6px 14px rgba(15,163,168,.28); }
        .step h5{ margin:8px 0 6px; font-size:1.05rem; }
        .step p{ margin:0; color:var(--muted); }

        .testimonials{ margin-top:28px; display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:16px; }
        .tcard{ background:var(--card); border:1px solid var(--border); border-radius:14px; padding:16px; box-shadow:var(--shadow); }
        .tcard .who{ margin-top:10px; font-weight:700; color:var(--text); }

        .quick-links{ margin-top:24px; display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:16px; }
        .q{ background:var(--card); border:1px solid var(--border); border-radius:14px; padding:14px; box-shadow:var(--shadow); text-align:center; }
        .q a{ font-weight:800; }

        .newsletter{ margin-top:28px; padding:18px; border-radius:14px; background:linear-gradient(120deg, var(--primary) 0%, var(--laser) 50%, var(--info) 100%); color:#fff; box-shadow:0 12px 32px rgba(5,41,45,.18); display:flex; gap:12px; align-items:center; justify-content:space-between; flex-wrap:wrap; }
        .newsletter input[type="email"]{ width:260px; max-width:100%; padding:10px 12px; border-radius:10px; border:0; }

        footer.alt-footer{ margin:36px 0 8px; text-align:center; color:var(--muted); }
        ...
        <div class="inner">
            <div>
                ...
                <div class="mini-widgets">
                    ...
                    <div class="mini">
                        <h5>Community</h5>
                        <p><strong><?php echo e($users_display); ?></strong> members<?php if ($fulfilled_rate !== null) { echo " • ".e($fulfilled_rate)."% fulfilled"; } ?></p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section class="features">
        <div class="tile">
            <div class="icon" aria-hidden="true">
                <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M3 7a2 2 0 012-2h4l2 2h8a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V7z"/></svg>
            </div>
        ...
        <div class="tile">
            <div class="icon" aria-hidden="true">
                <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M12 7a5 5 0 00-5 5H5l3.5 3.5L12 12h-2a3 3 0 116 0 6 6 0 11-6-6z"/></svg>
            </div>
        ...
        <div class="tile">
            <div class="icon" aria-hidden="true">
                <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M12 13c2.21 0 4-1.79 4-4S14.21 5 12 5 8 6.79 8 9s1.79 4 4 4zm0 2c-3.31 0-6 2.69-6 6h2a4 4 0 118 0h2c0-3.31-2.69-6-6-6z"/></svg>
            </div>
