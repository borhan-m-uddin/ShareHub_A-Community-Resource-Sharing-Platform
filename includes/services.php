...
if (!function_exists('service_get')) {
    function service_get(int $serviceId): ?array
    {
        ...
        if ($st = $conn->prepare('SELECT * FROM services WHERE service_id=? LIMIT 1')) {
...
if (!function_exists('service_create')) {
    function service_create(array $data): ?int
    {
        ...
        $avail = (string)($data['availability'] ?? 'available');
        $loc = (string)($data['location'] ?? null);
        if ($st = $conn->prepare($sql)) {
            $st->bind_param('isssss', $giver, $title, $desc, $cat, $avail, $loc);
            if ($st->execute()) {
                $id = (int)$st->insert_id;
                $st->close();
                return $id;
...
if (!function_exists('service_update')) {
    function service_update(int $serviceId, array $data): bool
    {
        ...
        $params = [];
        $types = '';
        foreach (['title', 'description', 'category', 'availability', 'location'] as $f) {
...
if (!function_exists('service_delete')) {
    function service_delete(int $serviceId, int $giverId): bool
    {
        ...
        $serviceId = (int)$serviceId;
        $giverId = (int)$giverId;
        if ($st = $conn->prepare('DELETE FROM services WHERE service_id=? AND giver_id=?')) {
            $st->bind_param('ii', $serviceId, $giverId);
            $ok = $st->execute();
            $st->close();
            return (bool)$ok;
        }
