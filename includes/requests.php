...
if (!function_exists('requests_list_for_user')) {
    function requests_list_for_user(int $userId, string $role = 'any', int $limit = 20, int $offset = 0): array
    {
        ...
        if ($role === 'requester') {
            ...
            $where[] = '(requester_id=' . $userId . ' OR giver_id=' . $userId . ')';
        }
...
if (!function_exists('request_create')) {
    function request_create(array $data): ?int
    {
        ...
        $req = (int)($data['requester_id'] ?? 0);
        $giver = (int)($data['giver_id'] ?? null);
        $item = (int)($data['item_id'] ?? null);
        $service = (int)($data['service_id'] ?? null);
        $type = (string)($data['request_type'] ?? 'item');
        $msg = (string)($data['message'] ?? null);
        $status = (string)($data['status'] ?? 'pending');
        if ($st = $conn->prepare($sql)) {
            $st->bind_param('iiiisss', $req, $giver, $item, $service, $type, $msg, $status);
            if ($st->execute()) {
                $id = (int)$st->insert_id;
                $st->close();
                return $id;
...
if (!function_exists('request_update_status')) {
    function request_update_status(int $requestId, string $status): bool
    {
        ...
        if ($st = $conn->prepare('UPDATE requests SET status=?, response_date=NOW() WHERE request_id=?')) {
            $st->bind_param('si', $status, $requestId);
            $ok = $st->execute();
            $st->close();
            return (bool)$ok;
        }
...
if (!function_exists('pair_can_message')) {
    function pair_can_message(int $userA, int $userB): bool
    {
        ...
        if ($st = $conn->prepare($sql)) {
            $st->bind_param('iiii', $userA, $userB, $userB, $userA);
            if ($st->execute()) {
                $f = $st->get_result()->fetch_row();
                $st->close();
                return (bool)$f;
            }
            $st->close();
