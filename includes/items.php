...
if (!function_exists('items_list')) {
    function items_list(array $filters = [], int $limit = 20, int $offset = 0): array
    {
        ...
        if (!empty($filters['status'])) { $where[] = 'availability_status=?'; $params[] = (string)$filters['status']; $types .= 's'; }
        ...
        if ($st = $conn->prepare($sql)) {
            ...
            if ($st->execute()) { $res = $st->get_result(); while ($r = $res->fetch_assoc()) { $rows[] = $r; } if($res){$res->free();} }
            $st->close();
        }
        return $rows;
    }
...
if (!function_exists('item_get')) {
    ...
    {
        if (!function_exists('db_connected') || !db_connected()) return null; global $conn; $itemId = (int)$itemId;
        if ($st = $conn->prepare('SELECT * FROM items WHERE item_id=? LIMIT 1')) { $st->bind_param('i', $itemId); if ($st->execute()) { $res=$st->get_result(); $r = $res->fetch_assoc(); if($res){$res->free();} $st->close(); return $r ?: null; } $st->close(); }
        return null;
    }
}
...
if (!function_exists('item_create')) {
    function item_create(array $data): ?int
    {
        ...
        $cond = (string)($data['condition_status'] ?? 'good'); $status = (string)($data['availability_status'] ?? 'available');
        $img = (string)($data['image_url'] ?? null); $loc = (string)($data['pickup_location'] ?? null);
        if ($st = $conn->prepare($sql)) { $st->bind_param('isssssss', $giver, $title, $desc, $cat, $cond, $status, $img, $loc); if ($st->execute()) { $id = (int)$st->insert_id; $st->close(); return $id; } $st->close(); }
        return null;
    }
}
...
if (!function_exists('item_update')) {
    ...
    {
        if (!function_exists('db_connected') || !db_connected()) return false; global $conn; $itemId = (int)$itemId;
        $fields = []; $params = []; $types='';
        foreach (['title','description','category','condition_status','availability_status','image_url','pickup_location'] as $f) {
            if (array_key_exists($f, $data)) { $fields[] = "$f=?"; $params[] = $data[$f]; $types .= 's'; }
        }
        if (!$fields) return true; $params[] = $itemId; $types .= 'i';
...
if (!function_exists('item_update_owned')) {
    function item_update_owned(int $itemId, int $giverId, array $data): bool
    {
        ...
        if (!$fields) return true; $params[] = $itemId; $params[] = $giverId; $types .= 'ii';
...
if (!function_exists('item_delete')) {
    ...
    {
        if (!function_exists('db_connected') || !db_connected()) return false; global $conn; $itemId=(int)$itemId; $giverId=(int)$giverId;
        if ($st = $conn->prepare('DELETE FROM items WHERE item_id=? AND giver_id=?')) { $st->bind_param('ii', $itemId, $giverId); $ok = $st->execute(); $st->close(); return (bool)$ok; }
        return false;
    }
}
