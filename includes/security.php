...
if (!function_exists('verification_generate_and_send')) {
    function verification_generate_and_send(int $userId, string $email, string $username = ''): bool
    {
        ...
        if ($stOld = $conn->prepare('UPDATE verification_tokens SET used_at=NOW() WHERE user_id=? AND used_at IS NULL')) {
            $stOld->bind_param('i', $userId);
            $stOld->execute();
            $stOld->close();
        }
        try {
        ...
        if ($st2 = $conn->prepare('INSERT INTO verification_tokens (user_id, token_hash, expires_at) VALUES (?,?,?)')) {
            $st2->bind_param('iss', $userId, $hash, $expires);
            $ok = $st2->execute();
            $st2->close();
            if ($ok) {
...
if (!function_exists('verification_mark_verified')) {
    function verification_mark_verified(int $userId, ?string $tokenHash = null): bool
    {
        ...
        global $conn;
        $ok = false;
        if ($st = $conn->prepare('UPDATE users SET email_verified=1 WHERE user_id=?')) {
            $st->bind_param('i', $userId);
            $ok = $st->execute();
            $st->close();
        }
        if ($ok && $tokenHash) {
            if ($st2 = $conn->prepare('UPDATE verification_tokens SET used_at=NOW() WHERE user_id=? AND token_hash=? AND used_at IS NULL')) {
                $st2->bind_param('is', $userId, $tokenHash);
                $st2->execute();
                $st2->close();
            }
        }
...
if (!function_exists('verification_is_verified')) {
    ...
    {
        if (!function_exists('db_connected') || !db_connected()) return true;
        global $conn;
        if ($st = $conn->prepare('SELECT email_verified FROM users WHERE user_id=? LIMIT 1')) {
            $st->bind_param('i', $userId);
            if ($st->execute()) {
...
if (!function_exists('verification_require')) {
    function verification_require(): void
    {
        ...
        if (!verification_is_verified((int)$_SESSION['user_id'])) {
            header('Location: ' . site_href('pages/verify_notice.php?uid=' . (int)$_SESSION['user_id']));
            exit;
        }
    }
}
