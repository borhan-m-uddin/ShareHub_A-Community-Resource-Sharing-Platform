...
if (!function_exists('upload_image_secure')) {
    function upload_image_secure(array $file, string $targetSubdir = 'uploads/items', int $maxBytes = 2_000_000, int $maxWidth = 1600, int $maxHeight = 1200): array
    {
        ...
        if (!$mime || !isset($allowed[$mime])) { return ['ok'=>false,'pathRel'=>null,'error'=>'Only JPG, PNG, GIF or WEBP images are allowed.']; }
        $ext = $allowed[$mime];
        ...
        $destAbs = $dirAbs . DIRECTORY_SEPARATOR . $name; $destRel = rtrim($targetSubdir, '/\\') . '/' . $name;
        if (!@move_uploaded_file($tmp, $destAbs)) { return ['ok'=>false,'pathRel'=>null,'error'=>'Failed to save uploaded image.']; }
        $canProcess = function_exists('imagecreatetruecolor') && function_exists('getimagesize');
        if ($canProcess && $ext !== 'gif') {
            $info = @getimagesize($destAbs); if (is_array($info) && !empty($info[0]) && !empty($info[1])) {
                $w = (int)$info[0]; $h = (int)$info[1];
                $scale = min($maxWidth / max(1,$w), $maxHeight / max(1,$h), 1.0);
                $reencode = function(string $extUse, $dest) use ($w,$h,$destAbs) {
                    $dst = imagecreatetruecolor($w,$h); $src = null;
                    if ($extUse==='jpg') { $src=@imagecreatefromjpeg($destAbs); }
                    elseif ($extUse==='png') { $src=@imagecreatefrompng($destAbs); imagealphablending($dst,false); imagesavealpha($dst,true); }
                    elseif ($extUse==='webp' && function_exists('imagecreatefromwebp')) { $src=@imagecreatefromwebp($destAbs); }
                    if ($src) { @imagecopy($dst,$src,0,0,0,0,$w,$h); if($extUse==='jpg'){ @imagejpeg($dst,$destAbs,85);} elseif($extUse==='png'){ @imagepng($dst,$destAbs,6);} elseif($extUse==='webp' && function_exists('imagewebp')) { @imagewebp($dst,$destAbs,85);} @imagedestroy($src); @imagedestroy($dst);}            
                };
                if ($scale < 1.0) {
                    $newW = max(1,(int)round($w*$scale)); $newH = max(1,(int)round($h*$scale));
                    $dst = imagecreatetruecolor($newW,$newH); $src=null;
                    ...
                    elseif ($ext==='webp' && function_exists('imagecreatefromwebp')) { $src=@imagecreatefromwebp($destAbs);}            
                    if ($src) { @imagecopyresampled($dst,$src,0,0,0,0,$newW,$newH,$w,$h); if($ext==='jpg'){ @imagejpeg($dst,$destAbs,85);} elseif($ext==='png'){ @imagepng($dst,$destAbs,6);} elseif($ext==='webp' && function_exists('imagewebp')) { @imagewebp($dst,$destAbs,85);} @imagedestroy($src); @imagedestroy($dst);}            
                } else { $reencode($ext,$destAbs); }
            }
        }
