...
if (!defined('PASSWORD_RESET_CODE_TTL')) {
    define('PASSWORD_RESET_CODE_TTL', 3600);
} // 1 hour
...
function password_reset_create(string $email): bool
{
    ...
    $now = time();
    if (!isset($_SESSION['pw_reset_requests'])) {
        $_SESSION['pw_reset_requests'] = [];
    }
    ...
    if ($stOld = $conn->prepare('UPDATE password_resets SET used_at=NOW() WHERE user_id=? AND used_at IS NULL')) {
        $stOld->bind_param('i', $uid);
        $stOld->execute();
        $stOld->close();
    }
    ...
    if ($stmt2 = $conn->prepare('INSERT INTO password_resets (user_id,email,otp_code,expires_at) VALUES (?,?,?,?)')) {
        ...
        if ($ok) {
            ...
            if (function_exists('send_email')) {
                ...
                $sent = App\Mail\Mailer::send($userEmail, 'Your ShareHub password reset code', $bodyHtml);
            }
            if (!$sent) {
                @file_put_contents($logFile, '[' . date('Y-m-d H:i:s') . "] password_reset_create: send FAIL for '$userEmail'\n", FILE_APPEND);
            } else {
                @file_put_contents($logFile, '[' . date('Y-m-d H:i:s') . "] password_reset_create: send OK for '$userEmail'\n", FILE_APPEND);
            }
        ...
        if ($stmt->execute()) {
            ...
            if ($row = $r->fetch_assoc()) {
                ...
                if ($row['used_at']) {
                    ...
                    $res = ['ok' => true, 'status' => 'ok', 'code' => $row['otp_code']];
                }
            } else {
                $res['status'] = 'notfound';
            }
