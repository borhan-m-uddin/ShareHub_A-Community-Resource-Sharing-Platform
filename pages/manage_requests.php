...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['request_id'], $_POST['action'])) {
    if (!csrf_verify($_POST['csrf_token'] ?? null)) {
        header('Location: ' . site_href('pages/manage_requests.php'));
        exit;
    }
    ...
    if (function_exists('db_connected') && db_connected()) {
        global $conn;
        $chkSql = "SELECT 1 FROM requests r LEFT JOIN items i ON r.request_type='item' AND r.item_id=i.item_id LEFT JOIN services s ON r.request_type='service' AND r.service_id=s.service_id WHERE r.request_id=? AND ((r.request_type='item' AND i.giver_id=?) OR (r.request_type='service' AND s.giver_id=?)) LIMIT 1";
        if ($st = $conn->prepare($chkSql)) {
            $st->bind_param('iii', $request_id, $uid, $uid);
            if ($st->execute()) {
    ...
    if ($allowed) {
        if (request_update_status($request_id, $new_status)) {
            if ($new_status === 'approved' && function_exists('db_connected') && db_connected()) {
                global $conn;
                if ($st2 = $conn->prepare("UPDATE items i JOIN requests r ON r.item_id=i.item_id AND r.request_type='item' SET i.availability_status='pending' WHERE r.request_id=?")) {
                    $st2->bind_param('i', $request_id);
                    $st2->execute();
                    $st2->close();
                }
            }
                        ...
                        <?php foreach ($incoming_requests as $request): ?>
                            <tr>
                                ...
                                <td class="status-<?php echo strtolower($request['status'] ?: 'pending'); ?>">
                                    <?php $status_icons = ['pending' => '⏳', 'approved' => '✅', 'rejected' => '❌', 'completed' => '🎯'];
