    ...
    if (!csrf_verify($_POST['csrf_token'] ?? null)) {
        ...
        switch ($action) {
            case 'add_service':
                ...
                if (!$title || !$description || !$location) { $error = 'Please fill in all required fields!'; }
                if (!$error) {
                    ...
                    if ($id) { $message = 'Service added successfully!'; } else { $error = 'Error adding service.'; }
                }
                break;
            case 'update_service':
                ...
                if (service_update_owned($service_id, $_SESSION['user_id'], [
                    ...
                    'location' => $location,
                ])) { $message = 'Service updated successfully!'; } else { $error = 'Error updating service.'; }
                break;
            case 'delete_service':
                $service_id = (int)($_POST['service_id'] ?? 0);
                if (service_delete($service_id, $_SESSION['user_id'])) { $message = 'Service deleted successfully!'; } else { $error = 'Error deleting service.'; }
                break;
        }
    }
}
