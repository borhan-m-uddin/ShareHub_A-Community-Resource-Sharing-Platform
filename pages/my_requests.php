    ...
    <?php if ($requests): ?>
        <div class="table-wrap">
            <table class="request-table">
                <thead><tr><th>Resource</th><th>Type</th><th>Message</th><th>Giver</th><th>Status</th><th>Request Date</th></tr></thead>
                <tbody>
                <?php foreach($requests as $r): $status=strtolower($r['status'] ?? 'pending'); ?>
                    <tr>
                        ...
                        <td class="status-<?php echo $status; ?>">
                            <?php $icons=['pending'=>'⏳','approved'=>'✅','rejected'=>'❌','completed'=>'🎯']; echo ($icons[$status] ?? 'ℹ️') . ' ' . htmlspecialchars(ucfirst($status)); ?>
                        </td>
