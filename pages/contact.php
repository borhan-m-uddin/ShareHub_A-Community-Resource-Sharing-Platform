...
$success_message = $_SESSION['flash_contact_success'] ?? null;
unset($_SESSION['flash_contact_success']);
$error_message = $_SESSION['flash_contact_error'] ?? null;
        ...
        if ($honeypot !== '' || $elapsed < $min_submit_seconds) {
            ...
            if ($name === '' || $email === '' || $subject === '' || $message === '') {
                ...
                if (function_exists('db_connected') && db_connected()) {
                    try {
                        @$conn->query("CREATE TABLE IF NOT EXISTS contact_messages (id INT AUTO_INCREMENT PRIMARY KEY,name VARCHAR(120) NOT NULL,email VARCHAR(254) NOT NULL,subject VARCHAR(200) NOT NULL,message TEXT NOT NULL,ip VARCHAR(45) NULL,user_agent VARCHAR(255) NULL,created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
