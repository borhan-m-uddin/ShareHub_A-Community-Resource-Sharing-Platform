...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrf_verify($_POST['csrf_token'] ?? null)) {
        ...
        if ($reviewed_user_id > 0 && $request_id > 0 && $comment !== '') {
            ...
            if ($st = $conn->prepare($check_sql)) {
                $st->bind_param('ii', $user_id, $request_id);
                $st->execute();
                $res = $st->get_result();
                if ($res->num_rows === 0) {
...
if ($st = $conn->prepare($sql)) {
    ...
    if ($st->execute()) {
        $res = $st->get_result();
        while ($row = $res->fetch_assoc()) $reviewable_requests[] = $row;
    }
    $st->close();
}
...
if ($st = $conn->prepare($sql)) {
    ...
    if ($st->execute()) {
        $res = $st->get_result();
        while ($row = $res->fetch_assoc()) $reviews_given[] = $row;
    }
    $st->close();
}
...
if ($st = $conn->prepare($sql)) {
    ...
    if ($st->execute()) {
        $res = $st->get_result();
        while ($row = $res->fetch_assoc()) $reviews_received[] = $row;
    }
    $st->close();
}
                                ...
                                <div class="list-item">
                                    <div class="rating">
                                        <div class="stars">
                                            ...
                                            <?php endfor; ?>
                                        </div><strong><?php echo $review['rating']; ?>/5</strong>
                                    </div>
