...
$requests_today = $requests_week = 0;
$fulfilled_rate = null;
...
if (function_exists('db_connected') && db_connected()) {
  ...
  try {
    ...
    if ($st = $conn->query("SELECT COUNT(*) c FROM items")) {
      $items_display = (string)($st->fetch_row()[0] ?? '0');
    }
    if ($st = $conn->query("SELECT title FROM items ORDER BY posting_date DESC LIMIT 1")) {
      $row = $st->fetch_row();
      if ($row) {
        $latest_item_t = $row[0];
      }
    }
    if ($st = $conn->prepare("SELECT COUNT(*) FROM items WHERE posting_date >= ?")) {
      $st->bind_param('s', $nowWeek);
      $st->execute();
      $r = $st->get_result()->fetch_row();
      $items_new_week = (int)($r[0] ?? 0);
      $st->close();
    }
  } catch (Throwable $e) {
  }
  ...
  try {
    $nowWeek = date('Y-m-d H:i:s', time() - 7 * 86400);
    if ($st = $conn->query("SELECT COUNT(*) c FROM services")) {
      $services_display = (string)($st->fetch_row()[0] ?? '0');
    }
    if ($st = $conn->query("SELECT title FROM services ORDER BY posting_date DESC LIMIT 1")) {
      $row = $st->fetch_row();
      if ($row) {
        $latest_serv_t = $row[0];
      }
    }
    if ($st = $conn->prepare("SELECT COUNT(*) FROM services WHERE posting_date >= ?")) {
      $st->bind_param('s', $nowWeek);
      $st->execute();
      $r = $st->get_result()->fetch_row();
      $services_new_week = (int)($r[0] ?? 0);
      $st->close();
    }
  } catch (Throwable $e) {
  }
  ...
  // Users count
  try {
    if ($st = $conn->query("SELECT COUNT(*) FROM users")) {
      $users_display = (string)($st->fetch_row()[0] ?? '0');
    }
  } catch (Throwable $e) {
  }
  ...
  try {
    ...
    if ($st = $conn->prepare("SELECT COUNT(*) FROM requests WHERE request_date >= ?")) {
      $st->bind_param('s', $today);
      $st->execute();
      $r = $st->get_result()->fetch_row();
      $requests_today = (int)($r[0] ?? 0);
      $st->close();
    }
    $weekAgo = date('Y-m-d H:i:s', time() - 7 * 86400);
    if ($st = $conn->prepare("SELECT COUNT(*) FROM requests WHERE request_date >= ?")) {
      $st->bind_param('s', $weekAgo);
      $st->execute();
      $r = $st->get_result()->fetch_row();
      $requests_week = (int)($r[0] ?? 0);
      $st->close();
    }
    if ($st = $conn->query("SELECT COUNT(*) f FROM requests WHERE status='completed'")) {
      $completed = (int)($st->fetch_row()[0] ?? 0);
    }
    if (!empty($requests_week) || !empty($requests_today)) {
      // approximate total vs completed ratio
      if ($st2 = $conn->query("SELECT COUNT(*) t FROM requests")) {
        $totalAll = (int)($st2->fetch_row()[0] ?? 0);
        if ($totalAll > 0 && isset($completed)) {
          $fulfilled_rate = round(($completed / $totalAll) * 100);
        }
      }
    }
  } catch (Throwable $e) {
  ...
  try {
    if ($st = $conn->query("SELECT title, posting_date FROM items ORDER BY posting_date DESC LIMIT 5")) {
  ...
  try {
    if ($st = $conn->query("SELECT title, posting_date FROM services ORDER BY posting_date DESC LIMIT 5")) {
      while ($r = $st->fetch_assoc()) {
        $latest_services[] = $r;
      }
    }
...
<head>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      if (!ul) return;
      ul.innerHTML = ul.innerHTML + ul.innerHTML; // duplicate for smooth scroll
    });
  </script>
</head>
...
<body>
  ...
  <section class="alt-wrapper">
    ...
    <div class="newsletter">
      <div><strong>Stay in the loop</strong>
    ...
    <div class="cta-band">
      <div><strong>Ready to start sharing?</strong>
