        ...
        switch ($action) {
            case 'add_item':
                ...
                if (isset($_FILES['image']) && is_array($_FILES['image']) && ($_FILES['image']['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE) {
                    ...
                    if ($res['ok']) {
                        $image_url = $res['pathRel'];
                    } else {
                        $error = $res['error'];
                    }
                }
                if (!$title || !$description || !$pickup_location) {
            ...
            <div class="modal-body">
                <form method="POST" id="editForm" enctype="multipart/form-data">
                    ...
                    <div class="grid" style="grid-template-columns:1fr 1fr; gap:16px;">
                        ...
                        <div class="form-group"><label for="edit_category">Category</label><select id="edit_category" name="category" class="form-control">
                                <option value="Electronics">Electronics</option>
                                <option value="Furniture">Furniture</option>
                                <option value="Clothing">Clothing</option>
                                <option value="Books">Books</option>
                        ...
                        <div class="form-group"><label for="edit_condition_status">Condition</label><select id="edit_condition_status" name="condition_status" class="form-control">
                                <option value="new">New</option>
                                <option value="like_new">Like New</option>
                        ...
                        <div class="form-group"><label for="edit_availability_status">Availability</label><select id="edit_availability_status" name="availability_status" class="form-control">
                        ...
                        <div class="form-group" style="grid-column:1 / -1;"><label for="edit_image">Change Image</label><input type="file" id="edit_image" name="edit_image" class="form-control" accept="image/*"><small class="muted">Leave empty to keep current image. Max 2MB. JPG/PNG/GIF/WEBP.</small>
                            <div style="margin-top:8px"><label style="display:flex;align-items:center;gap:8px;"><input type="checkbox" id="edit_remove_image" name="remove_image" value="1">Remove current image</label></div><img id="edit_current_image" src="" alt="Current image" style="margin-top:10px;max-width:100%;max-height:220px;display:none;border:1px solid var(--border);border-radius:6px;object-fit:cover;">
                        </div>
                    </div>
    ...
    <script>
        ...
        function deleteItem(id, title) {
            if (confirm('Are you sure you want to delete "' + title + '"? This action cannot be undone.')) {
                ...
                f.method = 'POST';
                f.innerHTML = `<?php echo str_replace('`', '\\`', csrf_field()); ?>` + '<input type="hidden" name="action" value="delete_item">' + '<input type="hidden" name="item_id" value="' + id + '">';
