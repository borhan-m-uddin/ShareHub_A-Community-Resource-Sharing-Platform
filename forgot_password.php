...
// Load flash data from previous redirect (if any)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrf_verify()) {
        ...
        if ($email === '' || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            // Always show generic success regardless of existence
            password_reset_create($email);
            $sent = true;
            $statusMsg = 'If that email is registered and verified, a reset code has been sent.';
        }
    }
    // Flash + redirect to prevent form resubmission / duplicate display on refresh
...
<div class="container" style="max-width:480px;margin:40px auto;">
    ...
    <?php else: ?>
        <p><a class="btn" href="login.php">Return to Login</a></p>
    <?php endif; ?>
</div>
</body>
</html>
