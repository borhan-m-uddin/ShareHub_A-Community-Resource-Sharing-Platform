				...
				<div class="notif-bell" id="notifBell" style="position:relative;display:inline-block;">
					...
					<div class="notif-menu" id="notifMenu" style="display:none;position:absolute;right:0;top:100%;">
						...
						<div id="notifList" class="menu-list">
							<div class="notif-item is-read"><div class="n-body">Loading...</div></div>
						</div>
...
(function(){
	...
	async function apiList(){
		loading = true;
		try {
			const r = await fetch('<?php echo site_href('pages/api/notifications.php'); ?>?action=list', {credentials:'same-origin'});
			const j = await r.json();
			if(!j.success){ listEl.innerHTML = '<div style="padding:12px 10px;font-size:13px;color:#b91c1c;">Failed: '+(j.message||'error')+'</div>'; return; }
			renderList(j.data||[]);
		} catch(e){
	...
	function formatDate(str){
		if(!str) return '';
		try { const d = new Date(str.replace(' ','T')); return d.toLocaleString(undefined,{month:'short', day:'numeric', hour:'2-digit', minute:'2-digit'}); } catch(e){ return str; }
	}
	...
	async function markAll(){
		...
		try {
			const r = await fetch('<?php echo site_href('pages/api/notifications.php'); ?>', {
				...
				body: JSON.stringify({action:'mark_all', csrf_token: csrfToken()})
			});
			const j = await r.json();
			if(j.success){
				// Re-render list as read
					bell.querySelectorAll('.notif-item').forEach(el=>{ el.style.opacity='.7'; });
	...
	function toggleMenu(e){
		...
		if(visible){ menu.style.display='none'; toggle.setAttribute('aria-expanded','false'); return; }
		menu.style.display='block'; toggle.setAttribute('aria-expanded','true');
		if(!loaded) apiList();
	}

	bell.addEventListener('click', function(ev){
		if(ev.target === toggle || toggle.contains(ev.target)) { toggleMenu(ev); }
	});
	document.addEventListener('click', function(ev){
		if(!bell.contains(ev.target)) { menu.style.display='none'; toggle.setAttribute('aria-expanded','false'); }
	});
