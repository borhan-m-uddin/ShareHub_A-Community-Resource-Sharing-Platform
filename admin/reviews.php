...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_review_id'])) {
    if (!csrf_verify($_POST['csrf_token'] ?? '')) {
        header('Location: ' . site_href('admin/reviews.php'));
        exit;
    }
...
// Count
$total = 0;
$sqlc = "SELECT COUNT(*) c FROM reviews r JOIN users ur ON r.reviewer_id=ur.user_id JOIN users uu ON r.reviewed_user_id=uu.user_id $where";
