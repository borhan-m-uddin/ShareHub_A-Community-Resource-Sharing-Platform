...
if ($st = $conn->prepare("SELECT availability_status, COUNT(*) as count FROM items GROUP BY availability_status")) {
    ...
    if ($res = $st->get_result()) {
        while ($row = $res->fetch_assoc()) {
            $stats['items_by_status'][$row['availability_status']] = (int)$row['count'];
        }
        $res->free();
    }
    $st->close();
...
if ($st = $conn->prepare("SELECT status, COUNT(*) as count FROM requests GROUP BY status")) {
    ...
    if ($res = $st->get_result()) {
        while ($row = $res->fetch_assoc()) {
            $stats['requests_by_status'][$row['status']] = (int)$row['count'];
        }
        $res->free();
    }
    $st->close();
        ...
        <div class="grid grid-3" style="margin-top:16px;">
            ...
            <div class="card">
                <div class="card-body" style="text-align:center;">
                    ...
                    <div style="margin-top:10px;"><span class="badge badge-available">Avg Rating: <?php echo $stats['average_rating']; ?>⭐</span></div>
                </div>
            </div>
        </div>
