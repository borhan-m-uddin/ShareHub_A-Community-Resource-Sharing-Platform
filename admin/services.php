...
if ($q !== '') {
    if (ctype_digit($q)) {
        $clauses[] = 's.service_id=?';
        $params[] = (int)$q;
        $types .= 'i';
    }
    $clauses[] = '(s.title LIKE ? OR s.description LIKE ? OR u.username LIKE ?)';
    $like = '%' . $q . '%';
    $params[] = $like;
    $types .= 's';
    $params[] = $like;
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (!csrf_verify($_POST['csrf_token'] ?? '')) {
        header('Location: ' . site_href('admin/services.php'));
        exit;
    }
...
// Count total
$total = 0;
$sqlc = "SELECT COUNT(*) c FROM services s JOIN users u ON s.giver_id=u.user_id $where";
