...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if ($action === 'delete' && $user_id === $self_id) {
            ...
            if ($res) { $res->bind_param('i', $user_id); $res->execute(); $res->close(); $notices[] = 'User status toggled.'; }
            ...
            if (empty($errors)) {
                ...
                // Messages
                if ($stmt = $conn->prepare("DELETE FROM messages WHERE sender_id = ? OR receiver_id = ?")) { $stmt->bind_param('ii', $user_id, $user_id); $stmt->execute(); $stmt->close(); }
                // Reviews (as reviewer or reviewed)
                if ($stmt = $conn->prepare("DELETE FROM reviews WHERE reviewer_id = ? OR reviewed_user_id = ?")) { $stmt->bind_param('ii', $user_id, $user_id); $stmt->execute(); $stmt->close(); }
                // Requests directly tied to user
                if ($stmt = $conn->prepare("DELETE FROM requests WHERE requester_id = ? OR giver_id = ?")) { $stmt->bind_param('ii', $user_id, $user_id); $stmt->execute(); $stmt->close(); }
                ...
                if ($stmt = $conn->prepare("SELECT item_id FROM items WHERE giver_id = ?")) {
                    ...
                    while ($row = $res->fetch_assoc()) {
                        $iid = (int)$row['item_id'];
                        if ($d = $conn->prepare("DELETE FROM requests WHERE item_id = ?")) { $d->bind_param('i', $iid); $d->execute(); $d->close(); }
                    }
                    $stmt->close();
                }
                if ($stmt = $conn->prepare("DELETE FROM items WHERE giver_id = ?")) { $stmt->bind_param('i', $user_id); $stmt->execute(); $stmt->close(); }
                ...
                if ($stmt = $conn->prepare("SELECT service_id FROM services WHERE giver_id = ?")) {
                    ...
                    while ($row = $res->fetch_assoc()) {
                        ...
                        if ($d = $conn->prepare("DELETE FROM requests WHERE service_id = ?")) { $d->bind_param('i', $sid); $d->execute(); $d->close(); }
                    }
                    $stmt->close();
                }
                if ($stmt = $conn->prepare("DELETE FROM services WHERE giver_id = ?")) { $stmt->bind_param('i', $user_id); $stmt->execute(); $stmt->close(); }
                // Finally delete user
                if ($stmt = $conn->prepare("DELETE FROM users WHERE user_id = ?")) { $stmt->bind_param('i', $user_id); $stmt->execute(); $stmt->close(); $notices[] = 'User deleted.'; }
            }
        }
    }
    }
...
if ($q !== '') {
    if (ctype_digit($q)) { $clauses[]='user_id=?'; $params[]=(int)$q; $types.='i'; }
    $clauses[]='(username LIKE ? OR email LIKE ?)'; $like='%'.$q.'%'; $params[]=$like; $types.='s'; $params[]=$like; $types.='s';
}
if ($role !== '') { $clauses[]='role=?'; $params[]=$role; $types.='s'; }
if ($statusF !== '') { $clauses[]='status=?'; $params[]=(int)$statusF; $types.='i'; }
if ($from !== '') { $clauses[]='DATE(registration_date) >= ?'; $params[]=$from; $types.='s'; }
if ($to !== '') { $clauses[]='DATE(registration_date) <= ?'; $params[]=$to; $types.='s'; }
...
// Count total
$total=0; $sqlc="SELECT COUNT(*) c FROM users $where"; if($st=$conn->prepare($sqlc)){ if($types!==''){ $st->bind_param($types,...$params);} if($st->execute()){ $r=$st->get_result(); $row=$r->fetch_assoc(); $total=(int)($row['c']??0); $r->free();} $st->close(); }
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        .tag { display:inline-block; padding:2px 6px; border-radius:4px; font-size:0.85em; background:var(--card); }
    </style>
    </head>
<body>
<?php render_header(); ?>
