...
if ($q !== '') {
    if (ctype_digit($q)) { $clauses[]='i.item_id=?'; $params[]=(int)$q; $types.='i'; }
    $clauses[]='(i.title LIKE ? OR i.description LIKE ? OR u.username LIKE ?)'; $like='%'.$q.'%'; $params[]=$like; $types.='s'; $params[]=$like; $types.='s'; $params[]=$like; $types.='s';
}
if ($statusF !== '') { $clauses[]='i.availability_status=?'; $params[]=$statusF; $types.='s'; }
if ($category !== '') { $clauses[]='i.category=?'; $params[]=$category; $types.='s'; }
if ($from !== '') { $clauses[]='DATE(i.posting_date) >= ?'; $params[]=$from; $types.='s'; }
if ($to !== '') { $clauses[]='DATE(i.posting_date) <= ?'; $params[]=$to; $types.='s'; }
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (!csrf_verify($_POST['csrf_token'] ?? '')) { header('Location: '.site_href('admin/items.php')); exit; }
    if (isset($_POST["delete_item_id"])) {
        $item_id = (int)$_POST["delete_item_id"];
        if ($stmt = $conn->prepare("DELETE FROM requests WHERE item_id = ?")) { $stmt->bind_param("i", $item_id); $stmt->execute(); $stmt->close(); }
        if ($stmt = $conn->prepare("DELETE FROM items WHERE item_id = ?")) {
            $stmt->bind_param("i", $item_id);
            if ($stmt->execute()) { header("location: " . site_href('admin/items.php')); exit; }
            $stmt->close();
        }
...
$total=0; $sqlc="SELECT COUNT(*) c FROM items i JOIN users u ON i.giver_id=u.user_id $where"; if($st=$conn->prepare($sqlc)){ if($types!==''){ $st->bind_param($types,...$params);} if($st->execute()){ $r=$st->get_result(); $row=$r->fetch_assoc(); $total=(int)($row['c']??0); $r->free();} $st->close(); }

$stats = ['available' => 0, 'unavailable' => 0, 'pending' => 0, 'total' => count($all_items)];
$categories = [];
foreach ($all_items as $it) {
    if (isset($stats[$it['availability_status']])) { $stats[$it['availability_status']]++; }
...
<div class="wrapper">
    ...
    <div class="grid grid-auto" style="margin-top:12px;">
        <div class="card"><div class="card-body" style="text-align:center;"><div style="font-weight:800;">Total Items</div><div style="font-size:1.4rem; font-weight:800; color:var(--primary);"><?php echo $stats['total']; ?></div></div></div>
        <div class="card"><div class="card-body" style="text-align:center;"><div style="font-weight:800;">Available</div><div class="status-available" style="font-size:1.4rem;"><?php echo $stats['available']; ?></div></div></div>
        <div class="card"><div class="card-body" style="text-align:center;"><div style="font-weight:800;">Pending</div><div class="status-pending" style="font-size:1.4rem;"><?php echo $stats['pending']; ?></div></div></div>
        <div class="card"><div class="card-body" style="text-align:center;"><div style="font-weight:800;">Unavailable</div><div class="status-unavailable" style="font-size:1.4rem;"><?php echo $stats['unavailable']; ?></div></div></div>
    </div>
