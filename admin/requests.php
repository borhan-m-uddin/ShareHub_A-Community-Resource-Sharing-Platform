    ...
    if ($rid > 0) {
        if ($action === 'delete') {
            ...
            if ($uStmt = $conn->prepare('SELECT requester_id, giver_id FROM requests WHERE request_id=? LIMIT 1')) {
                $uStmt->bind_param('i', $rid);
                if ($uStmt->execute()) {
                    $rres = $uStmt->get_result();
                    if ($row = $rres->fetch_assoc()) {
                        $reqId = (int)$row['requester_id'];
                        $giverId = (int)$row['giver_id'];
                        ...
                        if ($giverId && $newStatus === 'completed') {
                            @notify_user($giverId, 'request_status', 'Request #' . $rid . ' completed', $body, 'request', $rid, false);
                        }
                    }
                    if ($rres) $rres->free();
                }
...
if ($type !== '') {
    $clauses[] = 'r.request_type = ?';
    $params[] = $type;
    $types .= 's';
}
if ($from !== '') {
    $clauses[] = 'DATE(r.request_date) >= ?';
    $params[] = $from;
    $types .= 's';
}
if ($to !== '') {
